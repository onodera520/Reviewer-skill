...
反打可能让屏幕左右互换而世界位置不变。用柜台、入口、墙面等固定锚点验证，不按屏幕坐标直接判交换位置。角色手别按角色身体；镜像或遮挡使手别不明时标 uncertain。

普通货架记录“满载／部分陈列／空”等有意义状态，不逐件数普通商品；“两个便当袋”等剧情明确数量必须核对。允许普通包装、背景纹理、头发细节、曝光和微小阴影差异，不要求像素一致。
...
- 从明确 Spec／剧情建立 `basis: specified`；从实际清晰图片、且不与更权威要求冲突的事实建立 `observed`；两者均成立使用 `both`。
...
- 明确剧情或 Spec 描述的变化可以更新 value，证据记录变化来源。不能自行补“应该已经放下了”“可能有人清走了”。
...
以当前 Spec 核对当前合规，资产补充固定身份；Prompt 不能覆盖 Spec。Spec 和绑定资产有未解决冲突，先标 uncertain。前镜已经违反其 Spec 时不得以其错误实见作为正确基线。当前镜恢复正确 Spec 状态时把错误归属 previous，修复前镜。
...
同因问题只记录一次，用 fact_ids 链接有关事实；若也违反自身 Spec，用 related_review_areas 关联。修复建议仅恢复正确状态；不自动改 Prompt，不让画面改取景来隐藏矛盾。
